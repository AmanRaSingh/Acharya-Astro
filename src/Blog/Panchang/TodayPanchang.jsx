...
import { Grid, Typography, Paper, Box, TextField, Button, CircularProgress, Alert } from '@mui/material';
import axios from 'axios';

// Custom hook for fetching Panchang data
const usePanchang = (date, time, city) => {
  const [panchangData, setPanchangData] = useState(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  const fetchPanchang = async () => {
    console.log(`Fetching data from: https://acharyastro.com:5001/panchang/panchang?date=${date}&time=${time}&cityName=${city}`);
...
const TodayPanchang = () => {
  ...
  const { panchangData, loading, error, fetchPanchang } = usePanchang(date, time, city);
